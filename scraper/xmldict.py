...
def array_to_xml(array_xml, child_tag_name, root_tag_name='content'):
    declare = '<?xml version="1.0" encoding="UTF-8" ?>'
    return declare + '<' + root_tag_name + '>'+\
            '\n'.join(['<' + child_tag_name + '>'
                        + _to_xml(dict_xml)
                        + '</' + child_tag_name + '>' for dict_xml in array_xml]) +\
            '</' + root_tag_name + '>'

    #return '\n'.join([_to_xml(dict_xml) for dict_xml in array_xml])
