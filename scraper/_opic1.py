...
def visit_link(node, crawler=None, children_url_re=None, cache=None):
    ...
    if crawler is not None:
        ...
        demo_graph = [{'name':'A', 'children':['B', 'C', 'D']},
                      {'name':'B', 'children':['C',]},
                      {'name':'C', 'children':['A', 'E', 'F',]},
                      {'name':'D', 'children':['F', 'G']},
                      {'name':'E', 'children':['C',]},
                      {'name':'F', 'children':['G',]},
                      {'name':'G', 'children':['D', 'H']},
...
def keywords_occurrences(node, keywords_re):
    try:
...
def calculate_kopic(opic, kw_occurrences, total_kw_occurrences):
    if total_kw_occurrences != 0 and kw_occurrences != 0:
        kopic = opic * (3 + float(kw_occurrences)/total_kw_occurrences)
    else:
        kopic = opic
...
def start(domain='tuoitre.vn', template='<div id="divContent"><getme/></div>',
          max_nodes=100, max_added_nodes=10, keywords=(u'trung quốc',),
    ...
    while True:
        ...
        while True:
            ...
            log('Graph ', [(node['name'], node['cash'], node['history'],) for node in graph])
            log('Checked nodes ', [node for node in checked_nodes])
            foo = raw_input()
            node = max(graph, key=lambda x: x['cash'])
            log('Max node', node['name'])
            ...
            history = sum([node['history'] for node in graph])
            [compute_importance(node, history) for node in graph]
        ...
        log('Frontier', [(i['name'], i['cash']) for i in frontiers])
